 ...
 */
export type GenerateEventNames<
  Nouns extends readonly string[],
  Verbs extends Record<string, readonly string[]>,
> = {
  [N in Nouns[number]]: N extends keyof Verbs
    ? `${N}.${Verbs[N][number] extends string ? Verbs[N][number] : never}`
    : never
}[Nouns[number]]
