...
const (
\tdefaultBaseURL    = "${config.baseUrl || 'https://api.example.com.ai'}"
...
function generateGoMod(config: SDKConfig, goConfig?: GoSDKConfig): string {
  ...
  // If packageName looks like a Go module path (contains /), use it directly
  // Otherwise, use goConfig.modulePath or default to github.com/example/<packageName>
  let modulePath: string
  if (goConfig?.modulePath) {
    modulePath = goConfig.modulePath
  } else if (config.packageName.includes('/')) {
    // packageName is already a Go module path
    modulePath = config.packageName
  } else {
    modulePath = `github.com/example/${config.packageName}`
  }

  const goVersion = goConfig?.goVersion || '1.21'

  // For major version > 1, append /vN to module path
  const versionMatch = config.version.match(/^v?(\d+)\./)
  if (versionMatch) {
    const majorVersion = parseInt(versionMatch[1], 10)
    if (majorVersion >= 2 && !modulePath.endsWith(`/v${majorVersion}`)) {
      modulePath = `${modulePath}/v${majorVersion}`
    }
  }
...
function generateGoSum(): string {
  // Empty go.sum file - will be populated by go mod tidy
  return `// go.sum is auto-generated by go mod tidy
// Run 'go mod tidy' to populate this file
`
}

function generateReadme(nouns: ParsedNoun[], config: SDKConfig, goConfig?: GoSDKConfig): string {
  const modulePath = goConfig?.modulePath || `github.com/example/${config.packageName}`
  const pkgName = getPackageName(config.packageName)

  return `# ${config.packageName}

Go SDK for ${config.packageName} API.
...
func main() {
\tclient := ${pkgName}.NewClient(${pkgName}.ClientConfig{
\t\tAPIKey: "your-api-key",
\t})
...
\t// List resources
\tresult, err := client.${nouns[0]?.name || 'Users'}s.List(ctx, nil)
\tif err != nil {
\t\tpanic(err)
\t}
...
${nouns.map(n => `- \`${n.name}s\` - ${n.name} operations`).join('\n')}
...
// EventChannel is a channel that receives events.
type EventChannel chan *Event

// SubscriptionClient handles WebSocket subscriptions.
...
func NewSubscriptionClient(baseURL, apiKey string) *SubscriptionClient {
\tctx, cancel := context.WithCancel(context.Background())
\treturn &SubscriptionClient{
\t\tbaseURL:  baseURL,
\t\tapiKey:   apiKey,
\t\thandlers: make(map[string][]EventChannel),
\t\tctx:      ctx,
\t\tcancel:   cancel,
\t}
}
...
func (s *SubscriptionClient) Connect(ctx context.Context) error {
\twsURL := s.baseURL
...
\tconn, err := websocket.Dial(wsURL, "", s.baseURL)
\tif err != nil {
...
func (s *SubscriptionClient) listen(ctx context.Context) {
\tfor {
\t\tselect {
\t\tcase <-ctx.Done():
\t\t\treturn
...
\t\t\ts.mu.RLock()
\t\t\tchannels := s.handlers[event.Type]
\t\t\ts.mu.RUnlock()
...
// Subscribe creates a channel for receiving events of a specific type.
func (s *SubscriptionClient) Subscribe(ctx context.Context, eventType string) EventChannel {
\tch := make(EventChannel, 100)

\ts.mu.Lock()
\ts.handlers[eventType] = append(s.handlers[eventType], ch)
\ts.mu.Unlock()
...
\t\ts.mu.Lock()
\t\tdefer s.mu.Unlock()
\t\tchannels := s.handlers[eventType]
\t\tfor i, c := range channels {
\t\t\tif c == ch {
\t\t\t\ts.handlers[eventType] = append(channels[:i], channels[i+1:]...)
\t\t\t\tclose(ch)
\t\t\t\tbreak
\t\t\t}
\t\t}
...
func (s *SubscriptionClient) Disconnect() error {
\ts.cancel()
\tif s.conn != nil {
\t\treturn s.conn.Close()
\t}
...
export function generateGoFiles(
  ...
  files['subscriptions.go'] = generateSubscriptions(nouns, config)

  // Generate go.mod and go.sum
  files['go.mod'] = generateGoMod(config, goConfig)
  files['go.sum'] = generateGoSum()

  // Generate README
  files['README.md'] = generateReadme(nouns, config, goConfig)
