...
function generateSubscriptions(nouns: ParsedNoun[], config: SDKConfig): string {
  // Generate CRUD subscription methods for each noun
  const crudMethods = nouns.flatMap(noun => {
    const nounName = noun.name
    const camelName = toCamelCase(nounName)
    return [
      `
  /**
   * Subscribe to ${camelName} created events
   */
  onUserCreated(handler: EventHandler<${nounName}CreatedEvent>, options?: SubscriptionOptions): () => void {
    return this.subscribe('${camelName}.created', handler, options);
  }`,
      `
  /**
   * Subscribe to ${camelName} updated events
   */
  subscribeUserUpdated(handler: EventHandler<${nounName}UpdatedEvent>, options?: SubscriptionOptions): () => void {
    return this.subscribe('${camelName}.updated', handler, options);
  }`,
      `
  /**
   * Subscribe to ${camelName} deleted events
   */
  subscribeUserDeleted(handler: EventHandler<${nounName}DeletedEvent>, options?: SubscriptionOptions): () => void {
    return this.subscribe('${camelName}.deleted', handler, options);
  }`,
    ]
  }).join('')

  // Helper function to convert verb to past tense
  ...
  // Generate verb subscription methods for each noun
  const verbMethods = nouns.flatMap(noun => {
    const nounName = noun.name
    const camelName = toCamelCase(nounName)
    return noun.verbs.map(verb => {
      const pastTenseVerb = toPastTense(verb)
      const pascalPastTense = toPascalCase(pastTenseVerb)
      const pascalVerb = toPascalCase(verb)
      return `
  /**
   * Subscribe to ${camelName} ${verb} events
   */
  on${nounName}${pascalPastTense}(handler: EventHandler<${nounName}${pascalVerb}Event>, options?: SubscriptionOptions): () => void {
    return this.subscribe('${camelName}.${pastTenseVerb}', handler, options);
  }

  /**
   * Subscribe to ${camelName} ${verb} events (alias)
   */
  subscribe${nounName}${pascalPastTense}(handler: EventHandler<${nounName}${pascalVerb}Event>, options?: SubscriptionOptions): () => void {
    return this.subscribe('${camelName}.${pastTenseVerb}', handler, options);
  }`
    })
  }).join('')
  ...
  const eventTypes = nouns.flatMap(noun => [
    ...
    `${noun.name}UpdatedEvent`,
    `${noun.name}DeletedEvent`,
    ...noun.verbs.map(v => `${noun.name}${toPascalCase(v)}Event`)
  ])
...
import type { ${eventTypes.join(', ')} } from './types';
...
export class SubscriptionClient {
  ...
  private pendingSubscriptions: Array<{ eventType: string; filter?: Record<string, unknown> }> = [];
   ...
   */
  subscribe<T>(eventType: string, handler: EventHandler<T>, options?: SubscriptionOptions): Unsubscribe {
    ...
    const sub = { eventType, filter: options?.filter };
    ...
    return () => {
      ...
      const index = this.pendingSubscriptions.indexOf(sub);
      if (index !== -1) {
   ...
   */
  subscribeAll(events: Array<{ type: string; handler: EventHandler<unknown>; options?: SubscriptionOptions }>): Unsubscribe {
    const unsubscribes = events.map(({ type, handler, options }) =>
   ...
   */
  onEvents(eventTypes: string[], handler: EventHandler<unknown>, options?: SubscriptionOptions): Unsubscribe {
    const unsubscribes = eventTypes.map(type => this.subscribe(type, handler, options));
...
const DEFAULT_BASE_URL = '${config.baseUrl || 'https://api.example.com.ai'}';
...
${resourceInitializations}
  ...
  /**
   * Update the API key without reinstantiating the client
   */
  setApiKey(apiKey: string): void {
    this._apiKey = apiKey;
...
function generatePackageJson(config: SDKConfig, tsConfig?: TypeScriptSDKConfig): string {
  ...
  // Add publishConfig if provided
  if (tsConfig?.publishConfig) {
    pkg.publishConfig = tsConfig.publishConfig
  }
...
function generateReadme(nouns: ParsedNoun[], config: SDKConfig): string {
  const resourceExamples = nouns.slice(0, 2).map(noun => {
    const resourceName = noun.pluralName
    return `// ${noun.name} operations
const ${resourceName} = await client.${resourceName}.list();
const single = await client.${resourceName}.get('id');`
  }).join('\n')

  return `# ${config.packageName}

TypeScript SDK for ${config.packageName} API.
...
${nouns.map(n => `- \`${n.pluralName}\` - ${n.name} operations`).join('\n')}
...
export function generateTypeScriptFiles(
  ...
  files['package.json'] = generatePackageJson(config, tsConfig)
  files['tsconfig.json'] = generateTsConfig()
  files['tsup.config.ts'] = generateTsupConfig()
  files['README.md'] = generateReadme(nouns, config)
