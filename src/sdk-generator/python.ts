...
DEFAULT_BASE_URL = "${config.baseUrl || 'https://api.example.com.ai'}"
...
class Client:
    ...
    def __init__(
        ...
        Args:
            ...
            base_url: Base URL for API requests (default: ${config.baseUrl || 'https://api.example.com.ai'}).
...
function generatePyProject(config: SDKConfig): string {
  // Reserved for future pyproject.toml name configuration
  void config.packageName.replace(/-/g, '_')
...
function generateReadme(nouns: ParsedNoun[], config: SDKConfig): string {
  const resourceExamples = nouns.slice(0, 2).map(noun => {
    const resourceName = toSnakeCase(noun.pluralName)
    return `# ${noun.name} operations
${resourceName} = client.${resourceName}.list()
single = client.${resourceName}.get("id")`
  }).join('\n')

  return `# ${config.packageName}

Python SDK for ${config.packageName} API.
...
\`\`\`python
from ${config.packageName.replace(/-/g, '_')} import AsyncClient
...
${nouns.map(n => `- \`${toSnakeCase(n.pluralName)}\` - ${n.name} operations`).join('\n')}
...
class SubscriptionClient:
    """WebSocket subscription client for real-time events."""

    def __init__(self, base_url: str, api_key: str):
        self._base_url = base_url.replace("https://", "wss://").replace("http://", "ws://")
    ...
    async def connect(self) -> None:
        ...
        if websockets is None:
            raise ImportError("websockets package is required for subscriptions")

        ws_url = f"{self._base_url}/ws?apiKey={self._api_key}"
    ...
    async def on_user_created(
        ...
        return await self.subscribe("user.created", handler, filter)
...
@asynccontextmanager
async def subscribe(base_url: str, api_key: str):
    """Context manager for subscriptions."""
    client = SubscriptionClient(base_url, api_key)
...
export function generatePythonFiles(
  ...
  files['src/subscriptions.py'] = generateSubscriptions(nouns, config)
  ...
  files['MANIFEST.in'] = generateManifest()
